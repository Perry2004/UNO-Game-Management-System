...
const logError = (functionName) =>
  `OH NO! Error with ${functionName} in Dashboard Controllers:`;
const resError = (functionName) =>
  `OH NO! Internal Server Error with ${functionName} in Dashboard Controllers:`;
...
exports.loadDashboard = async (req, res) => {
  ...
  try {
    const [activePlayers, activeEvents, activeMatches, revenue, recentPlayers] =
      await Promise.all([
...
exports.checkFormInput = async (req, res) => {
  ...
  try {
    const usernameAvailable = await dashboardModel.isUsernameAvailable(
      username
    );
...
exports.updatePlayer = async (req, res) => {
  ...
  try {
    ...
    if (updates.username || updates.email) {
      await dashboardModel.updatePlayerUsernameAndEmail(
