...
const logError = (functionName) =>
  `OH NO! Error with ${functionName} in Store-Items Controllers:`;
const resError = (functionName) =>
  `OH NO! Internal Server Error with ${functionName} in Store-Items Controllers:`;
...
exports.fetchStoreItemsDetail = async (req, res) => {
  ...
  try {
    const results = await storeItemsModel.getStoreItemsDetailsByStoreID(
      storeID
    );
...
exports.fetchDiscount = async (req, res) => {
  ...
  try {
    const results = await storeItemsModel.getDiscountByAppliedPromotion(
      appliedPromotion
    );
    ...
    if (results == null) {
      return res
        .status(404)
        .send(`OH NO! Discount for ${appliedPromotion} does not exist!`);
    }
...
exports.checkFormInput = async (req, res) => {
  ...
  try {
    const itemNameAvailable = await storeItemsModel.isItemNameAvailable(
      itemName
    );
    ...
    const itemNotInPlayerStore = await storeItemsModel.isItemNotInPlayerStore(
      playerID,
      itemID
    );
...
exports.updateItem = async (req, res) => {
  ...
  try {
    ...
    if (updates.quality || updates.applied_promotion) {
      const originalPrice = await storeItemsModel.getOriginalPriceByQuality(
        quality
      );
      const discount = await storeItemsModel.getDiscountByAppliedPromotion(
        appliedPromotion
      );
...
exports.registerItem = async (req, res) => {
  ...
  try {
    ...
    await storeItemsModel.registerItem(
      name,
      quality,
      appliedPromotion,
      currentPrice
