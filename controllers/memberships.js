...
const logError = (functionName) =>
  `OH NO! Error with ${functionName} in Memberships Controllers:`;
const resError = (functionName) =>
  `OH NO! Internal Server Error with ${functionName} in Memberships Controllers:`;
...
exports.fetchPrivilegeClass = async (req, res) => {
  ...
  try {
    const results = await membershipsModel.getPrivilegeClassByPrivilegeLevel(
      privilegeLevel
    );
    ...
    if (results == null) {
      return res
        .status(404)
        .send(`OH NO! Privilege Class for ${privilegeLevel} does not exist!`);
    }
...
exports.fetchMembershipData = async (req, res) => {
  ...
  try {
    const results = await membershipsModel.getMembershipDataByPlayerID(
      playerID
    );
...
exports.checkMembershipExistence = async (req, res) => {
  ...
  try {
    ...
    const membershipExistence =
      await membershipsModel.isPlayerMembershipRegistered(playerID);

    if (!membershipExistence) {
      return res
        .status(200)
        .send(`OH YES! ${username} does not have a membership yet!`);
    }
...
exports.registerMembership = async (req, res) => {
  ...
  try {
    ...
    await membershipsModel.registerMembershipByPlayerID(
      playerID,
      duration,
      privilegeLevel
    );
