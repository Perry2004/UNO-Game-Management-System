...
const logError = (functionName) =>
  `OH NO! Error with ${functionName} in Memberships Models:`;
...
exports.getAllMemberships = async (order) => {
  ...
  switch (order) {
    ...
    case "daysRemaining":
      orderByClause =
        "membershipDaysRemaining DESC, m.status, m.issue_date DESC";
      break;
    case "privilegeLevel":
  ...
  try {
    ...
    return results.map((element) => ({
      ...
      membershipIssueDate: formatInTimeZone(
        element.membershipIssueDate,
        vancouverTimeZone,
        "yyyy-MM-dd"
      ),
      membershipExpireDate: formatInTimeZone(
        element.membershipExpireDate,
        vancouverTimeZone,
        "yyyy-MM-dd"
      ),
...
exports.getMembershipDataByPlayerID = async (playerID) => {
  try {
    ...
    results[0].membershipIssueDate = formatInTimeZone(
      results[0].membershipIssueDate,
      vancouverTimeZone,
      "yyyy-MM-dd"
    );
    results[0].membershipExpireDate = formatInTimeZone(
      results[0].membershipExpireDate,
      vancouverTimeZone,
      "yyyy-MM-dd"
    );
...
exports.isPlayerMembershipRegistered = async (playerID) => {
  try {
    const [results] = await db
      .promise()
      .query("SELECT membership_id FROM Memberships WHERE player_id = ?", [
        playerID,
      ]);
...
exports.updateMembershipByPlayerID = async (playerID, updates) => {
  try {
    ...
    if (updates.expire_date) {
      ...
      const expireDateUTC = toZonedTime(
        `${updates.expire_date}T00:00:00`,
        vancouverTimeZone
      );
      ...
      const issueDateVancouver = formatInTimeZone(
        issueDateUTC,
        vancouverTimeZone,
        "yyyy-MM-dd"
      );
      ...
      const expireDateVancouver = formatInTimeZone(
        expireDateUTC,
        vancouverTimeZone,
        "yyyy-MM-dd"
      );
...
exports.registerMembershipByPlayerID = async (
  playerID,
  duration,
  privilegeLevel
) => {
...
exports.deleteMembershipByPlayerID = async (playerID) => {
  try {
    await db
      .promise()
      .query("DELETE FROM Memberships WHERE player_id = ?", [playerID]);
...
async function updateMembershipStatus() {
  try {
    ...
    results.forEach(async (element) => {
      ...
      await db
        .promise()
        .query("UPDATE Memberships SET status = ? WHERE player_id = ?", [
          status,
          element.player_id,
        ]);
    });
