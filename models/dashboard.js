...
const logError = (functionName) =>
  `OH NO! Error with ${functionName} in Dashboard Models:`;
...
exports.getPlayerIDByUsername = async (username) => {
  try {
    const [results] = await db
      .promise()
      .query("SELECT player_id FROM Players WHERE username = ?", [username]);
...
exports.isUsernameAvailable = async (username) => {
  try {
    const [results] = await db
      .promise()
      .query("SELECT * FROM PlayerUsernameAndEmail WHERE username = ?", [
        username,
      ]);
...
exports.isEmailAvailable = async (email) => {
  try {
    const [results] = await db
      .promise()
      .query("SELECT * FROM PlayerUsernameAndEmail WHERE email = ?", [email]);
...
exports.updatePlayerUsernameAndEmail = async (
  newUsername,
  newEmail,
  oldUsername
) => {
