...
exports.updateEvent = async (name, eventStartDate, eventEndDate, numOfParticipants, eventStatus, eventID) => {
	...
	try {
		await db.promise().query("UPDATE Events SET name = ?, start_date = ?, end_date = ?, num_of_participants = ?, status = ? WHERE event_id = ?", [
			name,
			eventStartDate,
			eventEndDate,
			numOfParticipants,
...
exports.deleteEvent = async (eventID) => {
	...
	try {
		await db.promise().query("DELETE FROM Events WHERE event_id = ?", [eventID]); // parseInt() unneeded
	} catch (error) {
		// error_cf = error.message;
