...
const logError = (functionName) =>
  `OH NO! Error with ${functionName} in Store-Items Models:`;
...
exports.getStoreIDByPlayerID = async (playerID) => {
  try {
    const [results] = await db
      .promise()
      .query("SELECT store_id FROM Stores WHERE player_id = ?", [playerID]);
...
exports.deleteStoreItem = async (storeID, itemID) => {
  try {
    await db
      .promise()
      .query(`DELETE FROM StoreSellItems WHERE store_id = ? AND item_id = ?`, [
        storeID,
        itemID,
      ]);
